...
function resetAbandonedGenerationStatus() {
    try {
        ...
        const rawStatus = fs.readFileSync(statusFilePath, "utf-8");
        ...
        if (
            ...
            const resetStatus = {
                ...
                error: "La génération précédente a été interrompue par l’arrêt de l’application.",
                waitReason: null,
                retryAt: null,
            };
        ...
        console.error("[meal-generator-status-reset-error]", error);
    }
}
...
export async function POST() {
    ...
    try {
        ...
        processPython.stdout.on("data", (data: Buffer) => {
            console.log(`[meal-generator] ${data.toString().trim()}`);
        });

        processPython.stderr.on("data", (data: Buffer) => {
            console.error(`[meal-generator-error] ${data.toString().trim()}`);
        });

        processPython.on("error", (error) => {
            console.error("[meal-generator-spawn-error]", error);
        ...
        console.error("[meal-generator-start-error]", error);
