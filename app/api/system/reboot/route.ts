...
export async function POST(request: NextRequest) {
    ...
    const projectDirectory =
        process.env.LIFEBOARD_DIR ?? "/home/lifeboard/Desktop/Lifeboard";
    ...
    try {
        ...
        updateProcess.once("error", (error) => {
            ...
            console.error("[lifeboard-update-spawn-error]", error);
        });

        updateProcess.once("exit", (code, signal) => {
            if (code !== 0) {
                console.error("[lifeboard-update-exit-error]", {
                    code,
                    signal,
                });
            }
        ...
        console.error("[lifeboard-update-start-error]", error);
