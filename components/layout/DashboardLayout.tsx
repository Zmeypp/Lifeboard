...
export default function DashboardLayout() {
  ...
  async function loadStatus() {
    ...
    if (!data.isGenerating && data.mealPlan === 1 && data.images === 7) {
 ...
 useEffect(() => {
  ...
  useEffect(() => {
    ...
    const savedSnapshots = localStorage.getItem("lifeboard_net_worth_snapshots");
  ...
  useEffect(() => {
    if (!isLoaded) return;
    localStorage.setItem("lifeboard_operations", JSON.stringify(operations));
  }, [operations, isLoaded]);

  useEffect(() => {
    if (!isLoaded) return;
    localStorage.setItem("lifeboard_net_worth_snapshots", JSON.stringify(netWorthSnapshots));
    ...
    >
      {activePage === "overview" && (
        <DashboardGrid
          ...
          setNetWorthSnapshots={setNetWorthSnapshots}
      ...
      {activePage === "budget" && (
        <BudgetPage budgets={budgets} onUpdateBudgets={setBudgets} />
      )}
