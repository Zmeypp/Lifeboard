...
export default function OperationForm({
    ...
    useEffect(() => {
        if (!spendingBudgets.some((budget) => budget.id === selectedBudgetId)) {
            setSelectedBudgetId(spendingBudgets[0]?.id ?? "");
        }

        if (!accountBudgets.some((budget) => budget.id === selectedAccountId)) {
    ...
    function handleSubmit() {
        ...
        if (type === "expense") {
            if (expenseImpactMode === "budget") {
                ...
                if (!selectedBudget || !selectedBudget.linkedAccountId) {
                    return;
                }
                ...
                budgetImpact[selectedBudget.id] = -parsedAmount;

                accountImpact[linkedAccount.id] = -parsedAmount;
            ...
            if (expenseImpactMode === "account") {
                ...
                accountImpact[selectedAccount.id] = -parsedAmount;

                operationTitle = description.trim()
                    ? `${category} - ${description.trim()}`
                    : category;

                operationIcon = category === "Imprévus" ? "⚠️" : getIcon();
            }
        }
        ...
        if (type === "transfer") {
            if (
                !selectedAccountId ||
                !transferDestinationId ||
                selectedAccountId === transferDestinationId
            ) {
                return;
            }
            ...
            accountImpact[sourceAccount.id] = -parsedAmount;

            accountImpact[destinationAccount.id] = parsedAmount;

            operationTitle = `${sourceAccount.name} → ${destinationAccount.name}`;
        ...
        onAddOperation({
            ...
            type,
            category:
                type === "expense" && expenseImpactMode === "budget"
                    ? selectedBudgetId
                    : category,
            icon: operationIcon,
                ...
                <div className="space-y-4">
                    <div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <AnimatedButton
                                type="button"
                                onClick={() => setExpenseImpactMode("budget")}
                                className={`rounded-xl border px-4 py-3 font-semibold transition ${
                            ...
                            <AnimatedButton
                                type="button"
                                onClick={() => setExpenseImpactMode("account")}
                                className={`rounded-xl border px-4 py-3 font-semibold transition ${
