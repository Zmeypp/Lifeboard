...
export default function BudgetList({
    ...
    const currentCycleOperations = operations.filter((operation) =>
        isInCurrentBudgetCycle(operation.createdAt, settings.budgetResetDay),
    );
    return (
        <div className="space-y-4">
            {budgets.map((budget) => {
                const budgetImpact = currentCycleOperations.reduce(
                    (total, operation) =>
                        total + (operation.budgetImpact[budget.id] ?? 0),
                    0,
                );

                const accountImpact = currentCycleOperations.reduce(
                    (total, operation) =>
                        total + (operation.accountImpact[budget.id] ?? 0),
                    0,
                );
                ...
                const operationImpact =
                    budget.type === "account" ? accountImpact : budgetImpact;

                const remaining = Math.max(budget.amount + operationImpact, 0);
                ...
                const percent =
                    max > 0
                        ? Math.min(Math.round((remaining / max) * 100), 100)
                        : 0;
